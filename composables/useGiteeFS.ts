...
import type { GiteeConfig } from '~/types/file-system'

export type { GiteeConfig } from '~/types/file-system'
...
interface GiteeDirectoryResponse {
  name: string
  path: string
  sha: string
  type: 'file' | 'dir'
...
// 错误类型
export class GiteeFSError extends Error {
  constructor(
    message: string,
    public code: string,
    ...
    super(message)
    this.name = 'GiteeFSError'
  }
}

export function useGiteeFS(config: GiteeConfig) {
  ...
  // Gitee API 请求封装
  async function giteeRequest(path: string, options: RequestInit = {}): Promise<any> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          throw new GiteeFSError('Token 已失效，请重新授权', 'TOKEN_EXPIRED', 401)
        } else if (response.status === 403) {
          throw new GiteeFSError('权限不足，请检查 Token 权限', 'PERMISSION_DENIED', 403)
        } else if (response.status === 404) {
          throw new GiteeFSError('文件或目录不存在', 'NOT_FOUND', 404)
        } else if (response.status === 409) {
          throw new GiteeFSError('文件已存在或存在冲突', 'CONFLICT', 409)
        } else if (response.status === 422) {
          throw new GiteeFSError('请求参数错误: ' + message, 'VALIDATION_ERROR', 422)
        }

        throw new GiteeFSError(message, 'API_ERROR', response.status)
      }
      ...
      if (err instanceof GiteeFSError) {
        throw err
      }

      if (err instanceof TypeError && err.message.includes('fetch')) {
        throw new GiteeFSError('网络错误，请检查网络连接', 'NETWORK_ERROR')
      }

      throw new GiteeFSError(
  ...
  async function getFileContent(path: string): Promise<{ content: string; sha: string } | null> {
    try {
      ...
      const data = await giteeRequest(`/contents/${encodeURIComponent(fullPath)}?ref=${branch}`)
      ...
      // 使用 TextDecoder 正确处理 UTF-8 编码的中文
      ...
      if (err instanceof GiteeFSError && err.code === 'NOT_FOUND') {
        return null
      }
      throw err
    }
  ...
  async function createOrUpdateFile(
    ...
    await giteeRequest(`/contents/${encodeURIComponent(fullPath)}`, {
      method: 'POST',
  ...
  // 获取目录内容
  async function getDirectoryContents(path: string): Promise<GiteeDirectoryResponse[]> {
    try {
      const fullPath = getFullPath(path)
      const data = await giteeRequest(`/contents/${encodeURIComponent(fullPath)}?ref=${branch}`)
  ...
  const loadArticles = async (): Promise<Article[]> => {
    ...
    try {
      ...
      // （Gitee 存储不支持扫描目录初始化，因为没有 showDirectoryPicker）
  ...
  const saveImage = async (file: File | Blob, filename?: string): Promise<string> => {
    ...
    // 直接调用 giteeRequest 上传，避免 createOrUpdateFile 中的双重 base64 编码
    ...
    await giteeRequest(`/repos/${config.username}/${config.repo}/contents/${encodeURIComponent(getFullPath(path))}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    // 返回 Gitee raw URL
    return `https://gitee.com/${config.username}/${config.repo}/raw/${branch}/${getFullPath(path)}`
  }

  // 从剪贴板保存图片（Gitee 不支持直接访问剪贴板，返回 null）
  const saveImageFromClipboard = async (): Promise<string | null> => {
    // Gitee 存储模式不支持剪贴板直接上传
    // 需要在调用方处理剪贴板读取
    return null
  }
      ...
      for (const item of contents) {
        if (item.type === 'file') {
          ...
          mediaFiles.push({
            ...
            uploadTime: new Date().toLocaleString('zh-CN') // Gitee API 不返回上传时间
          })
        }
      }
    } catch (err) {
  ...
  const deleteMediaFile = async (filename: string): Promise<void> => {
    ...
    if (!result) {
      throw new GiteeFSError('文件不存在', 'NOT_FOUND', 404)
    }
